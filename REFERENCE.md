...
## Dependencies:
For minimal setup, have FireFly Compiler installed in C:\ffl and msys2 in C:\msys64
Python runtime, tested with v3.10 or higher
C compiler, works with msys2 mingw64
...
## Complete Example — Interactive Wave Simulation

A ripple simulation using a ping-pong wave equation on the GPU.
Click the window to drop ripples. Full source in `examples/wave.ff` and `examples/wave.fl`.

**`examples/wave.fl`** — two kernels: wave propagation and colour mapping:
...
**`examples/wave.ff`** — host program with ping-pong buffers and mouse input:
